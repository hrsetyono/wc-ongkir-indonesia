...
function wcis_get_fields_api( $params ) {
  ...
  if( isset( $params['city_id'] ) ) {
    $cities_field = preg_replace( '/(_city_field[\s\S]+)(\"' . $params['city_id'] . '\"\s)(\>)([\s\S]+\/p>)/Ui', '$1$2selected$3$4', $cities_field );
  }

  if( $params['district_id'] != '0' ) {
    $districts_field = preg_replace( '/(_district_field[\s\S]+)(' . $params['district_id'] . '\]\"\s)(\>)([\s\S]+\/p>)/Ui', '$1$2selected$3$4', $districts_field );
  }
...
function wcis_get_cities_api( $params ) {
  ...
  // If province code not given, show error message
    ...
    return [ 0 => __( 'Pilih Provinsi terlebih dahulu...' ) ];
  }
}
...
function wcis_get_districts_field( $type, $prov_code, $city_id = '0' ) {
  ...
  // Else, show the district selection
    ...
    $districts = wcis_get_districts_api( [ 'prov_code' => $prov_code, 'city_id' => $city_id ] );
    $field = woocommerce_form_field( "_{$type}_district", [
      'type' => 'select',
      'label' => __( 'Kecamatan' ),
      'options' => $districts,
...
function _wcis_read_json( $filename ) {
  $fileurl = WCIS_DIR . "/module-api/json/$filename";
  $args = array(
    'ssl' => array(
      'verify_peer' => false,
